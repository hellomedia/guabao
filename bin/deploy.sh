...
#
#     $ ssh deploy@taro.hellokot.be doas /data/kotmarket/activate-release.sh live <YEAR><MONTH><DATE>-<HOUR><MINUTE><SECOND>
# 
#     OR
#     $ cd /www/kotmarket/prod
#     $ rm current-release
...
USER=deploy
TRACK=${TRACK:-live}
PREFIX=/data/guabao/www/$TRACK
SSH="ssh"
RELEASE_NAME=$(date +"%Y%m%d-%H%M%S")
LOCAL_REPO=$(git rev-parse --show-toplevel || pwd)
REPO=git@github.com:hellomedia/guabao
DEST=$PREFIX/releases/$RELEASE_NAME
TMP_REPO=$(TMPDIR=$(pwd) mktemp -d -t guabao-deploy.XXXXX)
...
TAILWIND_BINARY_VERSION_DIR=$(basename $(dirname "$(realpath /home/nicolas/projects/guabao/var/tailwind/*/tailwindcss-linux-x64)"))
...
remote_activate () {
    if ! $SSH -t "$USER@$HOST" "doas /data/guabao/activate-release.sh $TRACK \"$(basename $DEST)\""; then
        set +x
